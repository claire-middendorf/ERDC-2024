...
#Gumbel Analysis
        ...
        count = 0 
        for i in floods: 
            hydrograph['{floods.loc[count,1]}-year Flood'] = unit_hydro['q/qp'].apply(
            lambda x: (x * floods.loc[count,2))
            count = count+1
        ...
        count = 0
        for i in hydrograph:   
            plt.plot(hydrograph['t'], hydrograph.loc[:,count), linestyle='-', label="1-year")
            count = count+1
