...
import pandas as pd
import floodfreq as ff
import k-table as fdata
...
def main():
    ...
    peak_flow_df2 = ff.process_peakflow_data(peak_flow_df)

    # step 3 - process peakflow stats; returns df and a dict
    peakflow_df3, summary_stats_dict = ff.process_peakflow_stats(peak_flow_df2)

    # step 4 - process coefficients
    coefficient_dict = ff.process_coefficients(skew_coeff=summary_stats_dict['skew_coeff'],
    ...
    k_df = pd.DataFrame.from_records(fdata.frequency_factors_records)
    ...
    # initiate class
    ff_class = ff.FrequencyFactors(k1=coefficient_dict['k1'],
